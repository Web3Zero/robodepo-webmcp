...
export function createRobodepoTools(options = {}) {
  ...
  function handoffAction(checkout) {
    return {
      ...
      why: "Fallback only, for the human, opened from the Robodepo page itself; a link opened from a chat will not carry the session.",
    };
  }
  ...
  async function createCheckout(input) {
    ...
    return buildEnvelope({
      ...
      instructions: {
        for_human:
          "Approve on the Robodepo page: look at the item, delivery region and total in the panel and touch once. Nothing is charged.",
        for_agent:
          "Do not open the approval link yourself: a link opened from a chat or another app carries no session. Tell the human to approve on the Robodepo page's panel with one touch. After they approve, call get_order with the checkout_id.",
      },
    });
  }
