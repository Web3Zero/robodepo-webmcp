...
After `create_checkout`, the agent hands the person a link to Robodepo's own approval page, showing the item, delivery region and total, nothing more, and asking for one confirmation, a WebAuthn presence check where the browser offers one, the same gesture as unlocking a phone with a fingerprint or face. It is not a payment authorisation and never touches the server; it just proves a person is at the device before the same form the plain button would submit gets submitted. Devices without a platform authenticator simply see the plain button, which works the same way.
...
The rule that matters most is simple to state and hard to get around: no tool can place an order. Only a person, on Robodepo's own approval page, in a real browser tab, can. Before that, the page already shows what the agent is doing: a live line under the hero updates with each tool call, and the activity panel opens itself the moment the first one arrives.

The mechanics come from the purchase contract. Opening the confirmation page requires the run cookie that owns that checkout, so a stranger who somehow learned a mandate's id still could not open it. Loading the page mints a fresh confirmation session: a short-lived cookie, a single-use security code, and a single-use order key, all issued together and thrown away the moment the page reloads or the checkout is used. The final submission must carry that exact code and key, not any value a caller invents, and the server checks the request came from Robodepo's own origin, not a copy of the page hosted elsewhere. The page refuses to load inside a frame, so it cannot be embedded invisibly and clicked by accident. Every check runs on the server, every time, regardless of what a tool or an agent claims a person did.
...
A regular product listing gives an agent a title, a price and a few bullet points: the same page a person would read, guessed at by a program instead of understood by one. Robodepo's aim is different: maximally truth-seeking product information, what an agent actually needs for an informed decision, not what a marketing page wants to show.

`get_product` already discloses the source retailer and both prices side by side, so an agent is never reasoning from one incomplete number. Ask for `include_evidence: true` and, for the demo product, a real pack comes back today: specifications, care, how to use it, review themes summarised rather than quoted, and transcript evidence from an independent YouTube review of the same hat in another colour, assembled from the retailer's product page and care guide, the brand's sourcing page and a corroborating listing, all fetched fresh. Every claim carries a source id, an honest gaps list says what could not be confirmed, and a freshness policy governs when it gets refetched. Every product, not just one, is the roadmap; where sources disagree, the pack says so rather than averaging it away.
...
None of this is invented for the pitch: `get_product`'s pricing and its evidence pack both work today; the full catalogue rollout and the shortlist are roadmap items, labelled as such wherever mentioned.

## How this is different from what exists

What exists today splits into two camps. Merchant-side tooling, Shopify's WebMCP, Google's UCP, Microsoft's Copilot Checkout, Perplexity's merchant program, Firmly's no-code connector, asks the store to do something first: install WebMCP-capable Liquid, connect a Merchant Center account, join a program, or run an onboarding flow, so a store that has never heard of agentic commerce is invisible to all of them. Buyer-side proxying, Amazon's Buy for Me, Rye and Zinc, automates one store's human checkout on the shopper's own saved details; no merchant cooperation needed, but it depends on that store's checkout form not changing.

Robodepo sits outside both camps:

- The shop does nothing. Robodepo aggregates instead of proxying one store at a time: it lists a store's products, discloses it as the source, and buys from it under the store's own published rules on the human's behalf, so a store that never installed WebMCP, never joined a program and never heard the word agent is still agent-completable, across every store Robodepo lists.
